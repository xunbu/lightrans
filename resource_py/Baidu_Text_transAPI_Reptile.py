...
class Dict:
    ...
    def loadMainPage(self):
        ...
        try:
            r = self.sess.get(url, headers=self.headers,proxies=proxy)
    ...
    def langdetect(self, query):
        ...
        proxy = {"http": None, "https": None}
        try:
            r = self.sess.post(url=url, data=data,proxies=proxy)
        except Exception as e:
            # raise e
            pass
    ...
    def dictionary(self, query, src=None, dst='zh'):
        """
            get translate result from https://fanyi.baidu.com/v2transapi
        """
        url = 'https://fanyi.baidu.com/v2transapi'

        sign = self.js.token(query, self.gtk)
        ...
        if r.status_code == 200:
            ...
            if 'error' in json:
                ...
                self.loadMainPage()
                self.loadMainPage()
                return {'error_code': json['error']}
            return json
        return None


    def baidu_trans_reptile(self,query,from_lang='auto',to_lang='zh'):
        return self.dictionary(query,src=from_lang,dst=to_lang)


d = Dict()

if __name__ == '__main__':
    print(d.baidu_trans_reptile("i love you \n do you love me"))
